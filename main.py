...
def extract_serial_numbers(internal_notes: str) -> list[str]:
    ...
    serials = re.findall(r'\b(SL\d{4,8}|\d{5,7})\b', internal_notes)
    return serials
...
class GPXScraper:
    ...
    def upload_csv(self, csv_path: str):
        ...
        try:
            # Step 1: Navigate directly to Devices page (sidebar may be collapsed)
            self.page.goto("https://admin.gpx.co/devices", wait_until="networkidle", timeout=15000)
            time.sleep(3)

            # Step 2: Click "Actions" button
            self.page.locator('button:has-text("Actions")').first.click(timeout=5000)
            time.sleep(1)

            # Step 3: Click "Update Retailer" from the dropdown
            self.page.locator('text=Update Retailer').first.click(timeout=5000)
            ...
            self.page.locator('button:has-text("Verify Data")').first.click(timeout=5000)
            ...
            update_btn = self.page.locator('button:has-text("Update Devices")').first
...
def run(target_date: str = None, dry_run: bool = False):
    ...
    # Filter out records with no serial number
    csv_records = [r for r in records if r["serial"] != "NOT FOUND"]
    log.info("Writing %d records to CSV (%d skipped — no serial).",
             len(csv_records), len(records) - len(csv_records))
