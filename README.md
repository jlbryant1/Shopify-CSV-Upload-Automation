...
Automated daily pipeline that pulls shipped Shopify orders from ShipStation, looks up device data (IMEI/ICCID) on the GPX admin portal, generates a CSV report in Google Drive, and uploads the data back to GPX to update retailer records.

## What It Does

1. **ShipStation** — Pulls all orders shipped today from a specific Shopify store using the ShipStation API
2. **Serial Extraction** — Parses internal notes on each order to extract 6-digit device serial numbers (orders without serials are excluded)
...
5. **GPX Retailer Update** — Uploads the same CSV back to GPX via the "Update Retailer" workflow to mark devices as sold through Shopify
